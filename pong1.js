...
const s =
  'Welcome to Infinte Pong! Player one controlls their paddle with the a and z keys. Player two controls their paddle with the /? key and the \'" key. Press the space bar to begin';
...
export const pong = p => {
  ...
  function ballBounceTopAndBottom() {
    ...
    if (
      ...
      speedY *= -1; // reverse the direction of the motion
      ball.y += speedY; // keeps things moving
      print('wham');
    }
  }
  function ballBounceRight() {
    ...
    if (
      ball.x + 12.5 > paddleR.x &&
      ball.y + 12.5 > paddleR.y &&
      ball.y + 12.5 < paddleR.y + paddleR.h &&
      ball.x > 0 &&
      ball.x < width &&
      ball.y > 0 &&
      ball.y < height
    ) {
      speedX *= -1; // This reverses the direction, I think
      ball.x += speedX; // This keeps the ball moving
      print('bam');
    } else if (
      ball.y + 12.5 > paddleR.y &&
      ball.y < paddleR.y + paddleR.y + paddleR.h &&
      ball.x + 12.5 > paddleR.x &&
      ball.x < paddleR.x + paddleR.x &&
      ball.x > 0 &&
      ball.x < width &&
      ball.y > 0 &&
      ball.y < height
  ...
  function ballBounceLeft() {
    ...
    if (
      ball.x - 12.5 < paddleL.x + paddleL.w &&
      ball.y + 12.5 > paddleL.y &&
      ball.y + 12.5 < paddleL.y + paddleL.h &&
      ball.x > 0 &&
      ball.x < width &&
      ball.y > 0 &&
      ball.y < height
